const replaceLogo = () => {
    ...
    if (element && element.getAttribute('logo-replaced') !== 'true') {
        ...
        removeTwitterPremium();
        observer.disconnect();
    }
}

const removeTwitterPremium = () => {
    const element = document.querySelector('a[aria-label="Verified"]') as HTMLLinkElement;
    while (element.firstChild) {
        element.removeChild(element.firstChild);
    }
};

const removeTwitterPremiumRight = () => {
    const element2 = document.querySelector('aside[aria-label="Subscribe to Premium"]') as HTMLLinkElement;
    if(element2 == null) return;
    element2.remove();
    rightSideObserver.disconnect();
}
const changeWebsiteTitle = () => {
...
const rightSideObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
        if (mutation.type === 'childList') {
            removeTwitterPremiumRight();
        }
    }
});

setInterval(changeWebsiteTitle, 100);
...
rightSideObserver.observe(document.body, {
    childList: true,
    subtree: true,
})